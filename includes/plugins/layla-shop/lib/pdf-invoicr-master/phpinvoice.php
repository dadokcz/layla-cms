...
class phpinvoice extends FPDF_rotation  {
    ...
    ******************************************/
	public function __construct($size='A4',$currency='$',$language='cs') {
		$this->columns  		  	= 3;
		$this->items 			  	= array();
		$this->totals 			  	= array();
		$this->addText 			  	= array();
	...
	public function setTransfer($date) {
		$this->transfer = $date;
	}
	...
	public function addItem($item,$description = "",$quantity,$vat,$price,$discount = 0,$total) {
		...
		//   $p['vat']			= $vat;
		//   if(is_numeric($vat)) {
		// 	  $p['vat']		=  number_format($vat,0,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency;
		//   } 
		// 	$this->vatField = false;
		// 	$this->columns = 5;		  
		// }
		...
		// 		$p['discount']	= number_format($discount,0,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency;
		// 	}
	...
	public function addTotal($name,$value,$colored = FALSE) {
		...
		$t['value']			= $value;
		if(is_numeric($value)) {
			$t['value']			= number_format($value,0,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency;
		} 
		$t['colored']		= $colored;
		$this->totals[]		= $t;
	}
	...
	public function Header() {
		...
		if(!empty($this->reference)) {
		  ...
		  $this->Cell(32,$lineheight,iconv("UTF-8", "UTF-8",strtoupper($this->lang['number']).':'),0,0,'L');
		...
		$this->SetFont($this->font,'B',9);
		$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		$this->Cell(32,$lineheight,iconv("UTF-8", "UTF-8",strtoupper($this->lang['date'])).':',0,0,'L');	
		$this->SetTextColor(50,50,50);
		$this->SetFont($this->font,'',9);
		...
		if(!empty($this->time)){
		  ...
		  $this->SetFont($this->font,'B',9);
		  $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		  $this->Cell(32,$lineheight,iconv("UTF-8", "UTF-8",strtoupper($this->lang['time'])).':',0,0,'L');	
		  $this->SetTextColor(50,50,50);
		  $this->SetFont($this->font,'',8.5);
		...
		if(!empty($this->due)){
			...
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(32,$lineheight,iconv("UTF-8", "UTF-8",strtoupper($this->lang['due'])).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
		...
		if(!empty($this->transfer)){
			...
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(32,$lineheight,iconv("UTF-8", "UTF-8",strtoupper($this->lang['transfer'])).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
			$this->Cell(0,$lineheight,$this->transfer,0,1,'R');
		}
		...
		if(!isset($this->productsEnded))  {
			...
			$this->Cell($this->firstColumnWidth,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['product'])),0,0,'L',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['qty'])),0,0,'C',0);
			if(isset($this->vatField)) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper('')),0,0,'C',0);
				// $this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['vat'])),0,0,'C',0);
			}
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['price'])),0,0,'C',0);
			if(isset($this->discountField)) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['discount'])),0,0,'C',0);
			}
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			// $this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper($this->lang['total'])),0,0,'C',0);
			$this->Cell($width_other,10,iconv("UTF-8", "UTF-8",strtoupper('')),0,0,'C',0);
			$this->Ln();
	...
	public function Body() {	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				$this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
				$this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252', number_format($item['price'],0,$this->referenceformat[0],$this->referenceformat[1])).' '.$this->currency,0,0,'C',1);
				// if(isset($this->discountField)) 
				// {
				// 	$this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
				...
				// }
				// $this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
				// $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252', number_format($item['total'],0,$this->referenceformat[0],$this->referenceformat[1])).' '.$this->currency,0,0,'C',1);
				$this->Ln();
		...
		if($this->totals) 
		{
			foreach($this->totals as $total) 
			{
				...
				$this->Cell($width_other-1,$cellHeight,iconv('UTF-8', 'UTF-8',$total['name']),0,0,'L',1);
				$this->Cell($this->columnSpacing,$cellHeight,'',0,0,'L',0);
				$this->SetFont($this->font,'b',8);
				$this->SetFillColor($bgcolor,$bgcolor,$bgcolor);
				if($total['colored']) 
				...
				$this->Cell($width_other,$cellHeight,iconv('UTF-8', 'UTF-8',$total['value']),0,0,'C',1);
				$this->Ln();
				$this->Ln($this->columnSpacing);
			}
		}
