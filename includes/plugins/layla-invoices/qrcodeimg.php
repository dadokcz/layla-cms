...
//$postid = get_the_guid($id);


// $url = $_GET['url'];
// $postid = url_to_postid($url);
// $invoice_num =  get_post_meta($postid, 'invoice_number', true);

$page = str_replace('/', '', $_GET['page']);

$get_invoice = @find_content(ROOT_ABSOLUTE_PATH.'data/invoices.csv', $page);

$invoice_num = $get_invoice['attributes']['invoice-id'];

if(!file_exists('./includes/plugins/layla-invoices/cache/qrcode-'.$invoice_num)){

$bankaccount = '1693035015/3030';

$message = "SPD*1.0*ACC:CZ0003000000000211057461*AM:".$bankaccount."*CC:CZK*X-VS:".$invoice_num."*MSG:FAKTURA DADOK #".$invoice_num."*NTA:info@dadok.cz*X-PER:2*";
...
$qr->draw(200, "includes/plugins/layla-invoices/cache/qrcode-".$invoice_num."");
// $qr->draw(100);
}
