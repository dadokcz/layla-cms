...
if($post['attributes']['payment_state'] != 'paid'){
...
if(file_exists('./includes/plugins/layla-shop-gateway-'.slugify($plugin_name).'/custom_button.php')){
  include './includes/plugins/layla-shop-gateway-'.slugify($plugin_name).'/custom_button.php';
}else{

  echo '    <button style="float:right; zoom: 0.85; margin-top: 0;" type="submit" class="button alt gradient" name="submit-'.slugify($plugin_name).'" id="submit-'.slugify($plugin_name).'" value="Objednat" data-value="Objednat">{{Pay through}} '.$plugin_name.'</button>';
