...
<div class="woocommerce-message">  {{No products in cart.}}</p></div>

                                <?php }else{?>

<div class="woocommerce-message">  <p>{{You have}} <?=cart_items_count();?> {{products in your cart}}.</p></div>
    ...
    <table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
        ...
        <input type="number" class="quantityInput input-text qty text" step="1" min="0" max="16" name="sku" value="<?=$value['sku'];?>" title="Množství" size="4" pattern="[0-9]*" inputmode="numeric"><input type="submit" value=">"></form>
                        ...
                        <td class="product-subtotal" data-title="Cena celkem">
                            <span class="woocommerce-Price-amount amount"><?=$post['attributes']['price']*$value['sku'];?>&nbsp;<span class="woocommerce-Price-currencySymbol">Kč</span></span>                      </td>
                    </tr>
