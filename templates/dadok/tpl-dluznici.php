...
<!-- 
                 <tr>
                <?php
                $timeZAKAZKY = strtotime("2021-06-18");
                $cena = 1620; 
                $pocetdni = round((time()-$timeZAKAZKY)/86400);
                $penale = $cena+(($cena*0.0025)*$pocetdni);
                // $penale = ($cena/100*0.15); 
                ...
                <td class="first"><strong>BressKamp s.r.o.</strong><br/>Jörg Fetting</td>
                <td>IT Služby a Ostatní, Místo výkonu: Polizy </td>
                <td><?=date("d/m/Y",$timeZAKAZKY);?></td>
                <td><?=$cena;?> Kč</td>
                <td class="red"><?=$penale;?> Kč<br/> <small>0.25% denně</small></td>
                 ...
                 <tr>
                <?php
                $timeZAKAZKY = strtotime("2019-09-01");
                $cena = 29990; 
                $pocetdni = round((time()-$timeZAKAZKY)/86400);
                $penale = $cena+(($cena*0.0025)*$pocetdni);
                // $penale = ($cena/100*0.15); 
