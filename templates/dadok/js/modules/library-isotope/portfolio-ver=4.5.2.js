...
function bringPortfolio(current_id){
	...
	// alert(global_current_id);

	// window.history.pushState('page2', 'Title', projectsArray[current_id][7]);

	jQuery('.portfolio_box').removeClass('portfolio_box-visible');

	jQuery('.tn-grid-container').hide();
	jQuery('.welcome-text').hide();
	// 
	...
	var descLong = projectsArray[current_id][11];
